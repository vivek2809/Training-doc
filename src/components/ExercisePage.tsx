...
import {
  Lightbulb, Eye, EyeOff, CheckCircle2, Circle,
  Clock, ChevronDown, ChevronUp, Trophy, Zap, Star, Target,
} from 'lucide-react';
...
// ─── Difficulty badge ────────────────────────────────────────────────────────
const DIFF: Record<Difficulty, { label: string; color: string; bg: string; icon: React.ReactNode }> = {
  beginner:     { label: 'Beginner',     color: 'text-emerald-400', bg: 'bg-emerald-400/10 border-emerald-400/30', icon: <Star size={11} /> },
  intermediate: { label: 'Intermediate', color: 'text-amber-400',   bg: 'bg-amber-400/10 border-amber-400/30',     icon: <Zap  size={11} /> },
  advanced:     { label: 'Advanced',     color: 'text-red-400',     bg: 'bg-red-400/10 border-red-400/30',         icon: <Target size={11} /> },
};

// ─── Day themes ──────────────────────────────────────────────────────────────
const DAY_THEME = {
  1: { gradient: 'from-emerald-500 to-teal-600',   accent: 'text-emerald-400', border: 'border-emerald-500/20', bg: 'bg-emerald-500/8',  check: 'bg-emerald-500 border-emerald-500', ring: 'ring-emerald-500/30' },
  2: { gradient: 'from-sky-500 to-blue-600',        accent: 'text-sky-400',     border: 'border-sky-500/20',     bg: 'bg-sky-500/8',      check: 'bg-sky-500 border-sky-500',         ring: 'ring-sky-500/30'     },
  3: { gradient: 'from-violet-500 to-purple-600',   accent: 'text-violet-400',  border: 'border-violet-500/20',  bg: 'bg-violet-500/8',   check: 'bg-violet-500 border-violet-500',   ring: 'ring-violet-500/30'  },
} as const;
  ...
  return (
    <div className="mt-3 rounded-lg overflow-hidden border border-white/8">
      <div className="flex items-center gap-2 px-3 py-1.5 bg-[#161b2e] border-b border-white/5">
        ...
        <span className="text-[10px] text-slate-500 uppercase tracking-widest" style={{ fontFamily: 'var(--font-mono)' }}>
          {lang ?? 'code'}
        </span>
      </div>
      <pre className="p-3 text-[12px] text-slate-300 overflow-x-auto leading-relaxed" style={{ fontFamily: 'var(--font-mono)' }}>
        <code>{code}</code>
      </pre>
    </div>
  );
...
function TaskCard({
  ...
  return (
    <div className={`rounded-xl border transition-all duration-200 overflow-hidden ${done ? 'border-white/10 bg-white/2 opacity-75' : 'border-white/8 bg-[#0d1117]'}`}>
      {/* Task header row */}
      <div className="flex items-start gap-3 p-4">
        ...
        <button
          ...
          className={`flex-shrink-0 mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
            done ? `${checkColor} border-transparent` : 'border-slate-600 hover:border-slate-400 bg-transparent'
          }`}
        >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-[10px] font-bold ${accentColor} opacity-70`} style={{ fontFamily: 'var(--font-mono)' }}>
              Task {index + 1}
            </span>
          </div>
          <p className={`text-[13px] leading-relaxed ${done ? 'line-through text-slate-500' : 'text-slate-300'}`}>
            {task.text}
          </p>
        </div>
      </div>
      ...
      {showHint && task.hint && (
        <div className="mx-4 mb-3 flex gap-2 bg-amber-400/8 border border-amber-400/20 rounded-lg p-3">
          ...
          <p className="text-[12px] text-amber-200 leading-relaxed">{task.hint}</p>
        </div>
      )}
      ...
      {showSolution && task.solution && (
        <div className="mx-4 mb-3">
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <span className="text-[10px] font-semibold text-sky-400 uppercase tracking-wider">Solution</span>
          </div>
          {task.solutionLang ? (
            <SolutionCode code={task.solution} lang={task.solutionLang} />
          ) : (
            <div className="bg-sky-400/8 border border-sky-400/20 rounded-lg p-3">
              <p className="text-[12px] text-sky-200 leading-relaxed">{task.solution}</p>
            </div>
          )}
        </div>
      )}
...
function ExerciseCard({ exercise, theme }: { exercise: Exercise; theme: typeof DAY_THEME[1] }) {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden transition-all duration-200 ${
      allDone ? 'border-emerald-500/30 bg-emerald-500/3' : 'border-white/8 bg-[#0d1117]'
    }`}>
      ...
      >
        ...
        <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center text-xl ${
          allDone ? 'bg-emerald-500/20 ring-2 ring-emerald-500/40' : 'bg-white/5'
        }`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap mb-1.5">
            <h3 className="font-semibold text-[15px] text-white" style={{ fontFamily: 'var(--font-display)' }}>
              {exercise.title}
            </h3>
            {allDone && (
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <span className={`flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border font-medium ${diff.color} ${diff.bg}`}>
              {diff.icon}
              {diff.label}
            </span>
          ...
          {done > 0 && (
            <div className="mt-2.5 flex items-center gap-2">
              ...
              <span className={`text-[10px] font-mono ${theme.accent}`}>{done}/{total}</span>
            </div>
          )}
        </div>
          ...
          {exercise.bonusChallenge && (
            <div className="mt-4">
              ...
              >
                ...
                {showBonus ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
              </button>
              {showBonus && (
                <div className="mt-2 bg-violet-500/8 border border-violet-500/20 rounded-xl p-4">
                  <p className="text-[13px] text-violet-200 leading-relaxed">{exercise.bonusChallenge}</p>
                </div>
              )}
            </div>
          )}
...
function ScoreBanner({ exercises, theme }: { exercises: Exercise[]; theme: typeof DAY_THEME[1] }) {
  const totalTasks     = exercises.reduce((a, e) => a + e.tasks.length, 0);
  const totalExercises = exercises.length;

  return (
    <div className={`rounded-2xl bg-gradient-to-br ${theme.gradient} p-6 mb-8 shadow-2xl`}>
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <p className="text-white/70 text-xs font-medium mb-1" style={{ fontFamily: 'var(--font-mono)' }}>
            Practice Lab
          </p>
        ...
        <div className="flex gap-4 flex-wrap">
          {(['beginner', 'intermediate', 'advanced'] as const).map(d => {
            ...
            return (
              <div key={d} className="text-center bg-white/10 rounded-xl px-4 py-2">
...
export default function ExercisePage({ day }: ExercisePageProps) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {/* How to use */}
      <div className={`rounded-xl border ${theme.border} ${theme.bg} p-4 mb-8 flex gap-3`}>
        <span className="text-lg flex-shrink-0">💡</span>
        <div>
          <p className="text-[13px] font-semibold text-white mb-1">How to use these exercises</p>
          <p className="text-[12px] text-slate-400 leading-relaxed">
            Click any exercise to expand it. Check off tasks as you complete them. Use the <strong className="text-amber-400">Hint</strong> button if you're stuck, and <strong className="text-sky-400">Show Solution</strong> to see the answer. Try to solve it yourself first!
          </p>
        </div>
      </div>
        ...
        <section key={topicId} className="mb-10">
          ...
          <div className={`flex items-center gap-2 mb-4 pb-2 border-b ${theme.border}`}>
            ...
            <span className="text-[10px] text-slate-600 ml-auto">
              {topicExercises.length} exercise{topicExercises.length !== 1 ? 's' : ''}
            </span>
          </div>
